...
dispersion_run_alias <- function(latitude, longitude, date_time,
  ...
  # Theese are the only *required* arguments to be provided via API
  burn <- data.frame(latitude, longitude, burn_dt = date_time)

  #' NOTE: The date_time field must be provided in a cannonical format that
...
# All plots together to prove alignmnet
test_runs %>%
  lapply(. %>%  `$`('disp_df')) %>%
  bind_rows() %>%
